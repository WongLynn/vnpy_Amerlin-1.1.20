...
# 注意这里直接使用的是引入的BarGenerator 和 ArrayManager 使用进行K线的合成与技术指标的合成的操作
from vnpy.trader.vtUtility import BarGenerator, ArrayManager  # 引入的k线以及指标的合成的类
...
class CtaTemplate(object):
    ...
    # ----------------------------------------------------------------------
    # 这种错误原因是将没有默认值的参数在定义时放在了有默认值的参数的后面,如下所示:
    def buy(self, vtSymbol, price, volume, stop, priceType=PRICETYPE_LIMITPRICE):
        """买开"""
        print('ctatemple 使用停止单突破 买开', price, volume)
        return self.sendOrder(CTAORDER_BUY, vtSymbol, price, volume, stop, priceType)

        # ----------------------------------------------------------------------

    def sell(self, vtSymbol, price, volume, stop, priceType=PRICETYPE_LIMITPRICE):
        """卖平"""
        print('ctatemple 使用停止单突破 卖开', price, volume)
        return self.sendOrder(CTAORDER_SELL, vtSymbol, price, volume, stop, priceType)

        # ----------------------------------------------------------------------

    def short(self, vtSymbol, price, volume, stop, priceType=PRICETYPE_LIMITPRICE):
        """卖开"""
        print('ctatemple 使用停止单突破 卖开', price, volume)
        return self.sendOrder(CTAORDER_SHORT, vtSymbol, price, volume, stop, priceType)

        # ----------------------------------------------------------------------

    def cover(self, vtSymbol, price, volume, stop, priceType=PRICETYPE_LIMITPRICE):
        """买平"""
        print('ctatemple 使用停止单突破 买平', price, volume)
        return self.sendOrder(CTAORDER_COVER, vtSymbol, price, volume, stop, priceType)

    # ----------------------------------------------------------------------
    def sendOrder(self, orderType, vtSymbol, price, volume, stop, priceType=PRICETYPE_LIMITPRICE):
        """发送委托"""
        if self.trading:
            # 如果stop为True，则意味着发本地停止单
            if stop:
                print("ctatemple 使用了突破买入止损单----》ctaenging")
                self.writeCtaLog('ctatemple 使用了突破买入止损单----》ctaenging')
                vtOrderIDList = self.ctaEngine.sendStopOrder(vtSymbol, orderType, price, volume, priceType, self)

                print("ctatemple 使用了突破买入止损单 ctaenging----》 vtOrderIDList ", vtOrderIDList)
                self.writeCtaLog('ctatemple 使用了突破买入止损单 ctaenging----》 vtOrderIDList %s' % (vtOrderIDList))
            else:
                print("ctatemple 直接发单 ctaenging----》 vtOrderIDList ", vtOrderIDList)
                vtOrderIDList = self.ctaEngine.sendOrder(vtSymbol, orderType, price, volume, priceType, self)
            return vtOrderIDList
        else:
            # 交易停止时发单返回空字符串
    ...
    def cancelAll(self):
        ...
        print("ctatemple cancleall 传入参数self.name", self.name)
        # ctatemple cancleall 传入参数self.name        BITFboll
    ...
    def loadHistoryBar(self, vtSymbol, type_, size=None, since=None):
        ...
        if type_ in ["1min", "5min", "15min", "30min", "60min", "120min", "240min", "360min", "480min", "1day", "1week",
                     "1month"]:
    ...
    def loadKindleBar(self, vtSymbol, type_, size=None, since=None):
        ...
        if type_ in ["1min", "5min", "15min", "30min", "60min", "120min", "240min", "360min", "480min", "1day", "1week",
                     "1month"]:
            data = self.ctaEngine.loadKindleBar(vtSymbol, type_, size, since)
            print('--------------------------------------加载历史数据-------------------------------------------------')
            print(data)
            """
                        --------------------------------------加载历史数据-------------------------------------------------
                           MTS         open        close         high          low      volume            datetime
            999  1560801300000  9302.800000  9324.500000  9326.600000  9300.100000   20.777715 2019-06-17 20:55:00
            998  1560801600000  9322.600000  9369.800000  9370.100000  9322.600000   54.731622 2019-06-17 21:00:00
            997  1560801900000  9369.900000  9377.900000  9383.000000  9364.014774   42.216412 2019-06-17 21:05:00
            996  1560802200000  9374.000000  9305.800000  9379.700000  9295.000000  158.742404 2019-06-17 21:10:00
            995  1560802500000  9304.600000  9325.600000  9340.000000  9300.000000   49.648955 2019-06-17 21:15:00
            994  1560802800000  9325.600000  9355.000000  9355.500000  9325.600000   42.225757 2019-06-17 21:20:00
            993  1560803100000  9354.200000  9344.600000  9355.200000  9320.200000   20.910815 2019-06-17 21:25:00
            992  1560803400000  9343.800000  9324.500000  9343.800000  9324.400000   10.383646 2019-06-17 21:30:00
            991  1560803700000  9324.500000  9310.700000  9324.500000  9285.000000   57.691973 2019-06-17 21:35:00
