...
class CtaTemplate(object):
    ...
    def buy(self, vtSymbol, price, volume, stop, priceType=PRICETYPE_LIMITPRICE):
        """买开"""
        if stop :
            self.ctaEngine.writeCtaLog('ctatemple 使用停止单突破 买开 price %s, volume %s'%(price, volume))
        else:
            self.ctaEngine.writeCtaLog('ctatemple 直接开单  买开 price %s, volume %s'%(price, volume))
        return self.sendOrder( vtSymbol,CTAORDER_BUY, price, volume, stop, priceType)

    def sell(self, vtSymbol, price, volume, stop, priceType=PRICETYPE_LIMITPRICE):
        """卖平"""
        if stop :
            self.ctaEngine.writeCtaLog('ctatemple 使用停止单突破 卖平 price %s, volume %s'%(price, volume))
        else:
            self.ctaEngine.writeCtaLog('ctatemple 直接开单  卖平 price %s, volume %s' % (price, volume))
        return self.sendOrder( vtSymbol,CTAORDER_SELL, price, volume, stop, priceType)


    def short(self, vtSymbol, price, volume, stop, priceType=PRICETYPE_LIMITPRICE):
        """卖开"""
        if stop :
            self.ctaEngine.writeCtaLog('ctatemple 使用停止单突破 卖开 price %s, volume %s'%(price, volume))
        else:
            self.ctaEngine.writeCtaLog('ctatemple 直接开单  卖开 price %s, volume %s' % (price, volume))
        return self.sendOrder( vtSymbol,CTAORDER_SHORT, price, volume, stop, priceType)

    def cover(self, vtSymbol, price, volume, stop, priceType=PRICETYPE_LIMITPRICE):
        """买平"""
        if stop :
            self.ctaEngine.writeCtaLog('ctatemple 使用停止单突破 买平 price %s, volume %s'%(price, volume) )
        else:
            self.ctaEngine.writeCtaLog('ctatemple 直接开单  买平 price %s, volume %s' % (price, volume))
        return self.sendOrder( vtSymbol,CTAORDER_COVER, price, volume, stop, priceType)

    # ----------------------------------------------------------------------
    def sendOrder(self, vtSymbol, orderType, price, volume, stop, priceType=PRICETYPE_LIMITPRICE):
        """发送委托"""
        if self.trading:
            if stop:
                self.writeCtaLog('ctatemple 使用了突破买入单类型')
                vtOrderIDList = self.ctaEngine.sendStopOrder( vtSymbol,orderType, price, volume, priceType, self)
                self.writeCtaLog('ctatemple 使用了突破买入 本地买入编号  vtOrderIDList %s' % (vtOrderIDList))
            else:
                self.writeCtaLog('ctatemple 使用直接发单类型')
                vtOrderIDList = self.ctaEngine.sendOrder( vtSymbol,orderType, price, volume, priceType, self)
                self.writeCtaLog('ctatemple 使用【直接买入】  本地买入编号  vtOrderIDList %s' % (vtOrderIDList))
