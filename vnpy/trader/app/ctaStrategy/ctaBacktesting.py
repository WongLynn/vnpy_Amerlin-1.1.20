...
class BacktestingEngine(object):
    ...
    def runParallelOptimization(self, strategyClass, optimizationSetting, strategySetting={}, prepared_data=[]):
        ...
        l = []
        # print("self.contracts",self.contracts)
        for setting in settingList:
            setting.update(strategySetting)
            self.clearBacktestingResult()                        # 清空策略的所有状态（指如果多次运行同一个策略产生的状态）
            l.append(pool.apply_async(optimize, (self.__class__, strategyClass, setting,targetName, self.mode,self.startDate, self.initHours, self.endDate,self.dbURI, self.bardbName, self.tickdbName, self.contracts)))
        pool.close()
        pool.join()
