...
class BasicMonitor(QtWidgets.QTableWidget):
    ...
    def saveToCsv(self):
        ...
        try:
            #if not path.isEmpty():
            if path:
                with file(unicode(path), 'wb') as f:
                    writer = csv.writer(f)
...
class OrderMonitor(BasicMonitor):
    ...
    def __init__(self, mainEngine, eventEngine, parent=None):
        ...
        d['offset'] = {'chinese':vtText.OFFSET, 'cellType':BasicCell}
        d['price'] = {'chinese':vtText.PRICE, 'cellType':NumCell}
        d['totalVolume'] = {'chinese':vtText.ORDER_VOLUME, 'cellType':NumCell}                             #委托数量
        d['tradedVolume'] = {'chinese':vtText.TRADED_VOLUME, 'cellType':NumCell}                           #成交量
