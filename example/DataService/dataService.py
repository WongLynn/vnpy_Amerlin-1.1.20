...
注意这里默认提供的下载的接口是缓存数据到分钟数据库collections
"""
...
# SYMBOL 的转换 注意这里将symbol 转换成为vtsymbol 即进行过数据库读写的,注意这里仅仅是对BITFINEX 进行了优化
def symbol_get(symbol):
    ...
    bar_symbol = l[0] + n[0]                                    # BTCUSD之前的symbol 是BTCUSD:BITFINEX 根据加载历史数据的函数现在是
    if n[-1] == 'BF':
...
def generateVtBar(symbol, d):                                    #传入交易对以及每条记录信息   "BTC/USD.BF"
    ...
    n = m.split('.')                                           # [USD,BF]
    bar = VtBarData()
    bar.symbol = l[0] + n[0]                                   # BTCUSD之前的symbol 是BTCUSD:BITFINEX 根据加载历史数据的函数现在是
    # bar.exchange = n[-1]
    ...
    """
    # 注意因为后边涉及到相关的计算，这里所有的包含数值的项目都是float 进行计算    根据原版之中的loadcsv 进行的处理

    bar.datetime = datetime.datetime.strptime(d['Time'],'%Y-%m-%dT%H:%M:%S.%f0Z')                    #匹配字符串格式，转换成时间格式
    bar.datetime = bar.datetime.strftime('%Y/%m/%d %H:%M')                                           #将时间格式转化成字符串格式，匹配输出自己想要的
    bar.datetime = datetime.datetime.strptime(bar.datetime,'%Y/%m/%d %H:%M')                         #再将字符串格式转换成 日期格式
...
class matrixdata_sdk():
    ...
    def downMinuteBarBySymbol(self, params):
        ...
        # 注意这里的数据库的表空间
        cl = db[_symbol]                                                           # 即定义的数据库1 min 数据库表空间
