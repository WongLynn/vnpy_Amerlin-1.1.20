...
class BollBandsStrategy(CtaTemplate):
        ...
        if engine == 'backtesting':
            ...
            for s in self.symbolList:
                for bar in kline60[s]:
                    # self.writeCtaLog(u' --加载历史数据- 60 bar------- %s' % bar.__dict__)
                    self.am60Dict[s].updateBar(bar)
                for bar in kline15[s]:
                    # self.writeCtaLog(u' --加载历史数据- 15 bar------- %s' % bar.__dict__)
                    self.am15Dict[s].updateBar(bar)
                for bar in kline1[s]:
                    # self.writeCtaLog(u' --加载历史数据-  bar------- %s' % bar.__dict__)
                    self.onBar(bar)
    ...
    def on60MinBar(self, bar):
        ...
        self.writeCtaLog(
            u' 收到合成 1 小时 BAR , bar.OPEN %s, bar.HIGH %s, bar.LOW %s, bar.CLOSE %s, DATETIME %s' % (bar.open, bar.high,
                                                                                                   bar.low, bar.close,
                                                                                                   bar.datetime))
        symbol = bar.vtSymbol
